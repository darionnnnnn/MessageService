...
public class HttpBaseAddressTests
{
    ...
    public void PlainUri_WithoutTrailingSlash_LosesTheSubApplicationPath()
    {
        ...
        var resolved = new Uri(new Uri("http://192.0.2.10/MSLine"), "api/edge/poll");

        Assert.Equal("http://192.0.2.10/api/edge/poll", resolved.ToString());
    }

    [Theory]
    [InlineData("http://192.0.2.10/MSLine")]
    [InlineData("http://192.0.2.10/MSLine/")]
    [InlineData("  http://192.0.2.10/MSLine  ")]
    public void Create_KeepsSubApplicationPath_RegardlessOfTrailingSlash(string configured)
    {
        ...
        Assert.Equal("http://192.0.2.10/MSLine/api/edge/poll", resolved.ToString());
    }
