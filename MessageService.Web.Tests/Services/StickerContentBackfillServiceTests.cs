...
public class StickerContentBackfillServiceTests : IDisposable
{
    ...
    [Fact]
    public async Task ExecuteAsync_BackfillsMissingContentRowsAndIsIdempotent()
    {
        using var scope = _provider.CreateScope();
        ...
        db.GroupMessages.AddRange(msg1, msg2, msg3, msg4);
        await db.SaveChangesAsync();

        var service = new StickerContentBackfillService(_provider.GetRequiredService<IServiceScopeFactory>(), NullLogger<StickerContentBackfillService>.Instance);

        await service.RunBackfillAsync(CancellationToken.None);

        // msg1 should get a pending content
        ...
        Assert.Equal(2, contentCount); // msg1 and msg4's contents
    }
}
