...
（用途／目標／請求網址／IP／結果／說明／經由）。「請求網址」是這次實際送出的絕對網址，
「IP」是目標 host 解析到的位址（IPv4 排前、解析不到顯示「解析失敗」）——這兩欄不論成功
失敗都顯示，拿去跟網管核對防火牆規則用。測試失敗同時寫入 log（Warning，含網址與 IP），
沒開著頁面事後也查得到：
...
目標欄與請求網址顯示的是 **proxy 的 host 與網址**（實際連的就是它，IP 也是 proxy 的）；此時後三列只證明「這台到 proxy 通」，
