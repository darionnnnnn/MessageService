...
（免重啟改機密與白名單）、**連線測試**（一鍵測 LINE outbound 用到的四個網域，直連與經
EdgeProxy 走同一條路徑，可用暫時的 token 只測不存）、**錯誤排查**（本機最近 200 則
...
- **同一個角色不支援部署兩台**（例如為了 HA 開兩台 Core）：維護類背景工作靠「每個角色恰好
  ...
  要橫向擴充只該擴 Viewer——但注意 Viewer 已經不是純讀：它有刪除群組／刪除歷史訊息的端點，
  多台同時對同一個群組操作沒有互斥（刪除本身冪等，第二台拿到 404）。
