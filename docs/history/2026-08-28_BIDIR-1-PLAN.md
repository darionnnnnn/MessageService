...
## 實測修正（2026-08-31，dev 上的 hotfix）

實機部署（Core 在 IIS 子應用 /MSLine、只開通 core→edge）抓到兩個測試矩陣沒涵蓋的真 bug：

| 症狀 | 根因 | 修法 |
|---|---|---|
| Core 輪詢一律 404 | `HttpClient.BaseAddress` 少結尾斜線時，RFC 3986 解析相對位址會把最後一段路徑換掉——IIS 子應用路徑整段消失。**推送方向的既有 client 有同樣缺陷**，只因 Core 裝在根目錄沒踩到 | `HttpBaseAddress.Create` 統一補結尾斜線，五處具名 client 全套用（`97414b2`） |
| poll 通了但訊息不出現 | A-step2 抽的共用述詞把 `NextAttemptAt`（推送退避排程）也套到拉取查詢——推送持續失敗時退避推到 300 秒後，Core 有長達五分鐘查不到訊息。**當初的測試把這個缺陷釘成契約**（斷言退避中的項目不出現在 poll） | 拆成 `WhereDeliverable`（共用，只排除死信）與 `WherePushDue`（推送專用）；反轉該測試（`173e589`） |

順手：心跳失敗告警節流（單向防火牆下送不到是穩態，原本每 60 秒噴完整堆疊）。

教訓：測試矩陣缺「**部署形狀**」維度——子應用路徑、推送持續失敗下的拉取，都是單機測試
造不出來的組合；且抽共用述詞時要問「兩個呼叫端的語意真的完全相同嗎」，把碰巧相同的
條件抽在一起，測試還會把錯的那半釘成契約。

收尾體檢（同模型，獨立性有限）：突變驗證核心修正、補心跳節流 2 測試、收斂兩處殘留的
inline 死信判定。結束時 919 綠。
