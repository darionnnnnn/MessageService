...
|---|---|---|---|---|
| A 訊息流活性 | agy | 通過 | 1034 綠（門檻 1033）、零警告；突變（`warnHours <= 0` 改 `< 0`）2 條紅 | 無落差 |
| B 加密設定檔三態＋監看 | agy | 通過（清掉兩個冗餘別名後） | 1045 綠（門檻 1042）、零警告；突變（停用 watcher）2 條 watcher 測試皆紅 | 見下方 |
| C 文件收斂 | Claude | 通過 | 跨段 grep：設定鍵四處一致、前端消費點命中、橫幅文案實作與測試一致 | 見終檢輪（文件 H1 是本段寫錯） |
| 終檢 | Claude | 通過 | 1045 綠、零警告；兩個獨立 Explore 各審程式碼與文件全 diff | 6 項修正，見下 |

## 終檢輪

### 文件終檢：一條寫錯的指引（高，已推翻重寫）

作業C 原本寫「用設定頁接管機密後，建議把 `appsettings` 的同名鍵清掉」——**這條是錯的，
而且會造成無法從 UI 復原的故障**。查證 `DeploymentValidator`：`ReceivesWebhook` 的模式
缺 `Line:ChannelSecret` 會直接 throw。所以清掉之後一旦加密檔讀不出來（搬機、還原映像），
設定值退回空的 appsettings → 啟動驗證擋下站台 → `/edge-admin` 連不上 →
同一段文件宣稱的復原路徑（重新填一次存檔）當場斷掉，只能改檔案救。

正確結論與原本相反：**`appsettings` 裡的機密是站台的開機備援，不要清空**。
留著舊值的意義不是它還能用（LINE 那邊已經換過，webhook 會 401），而是讓站台起得來、
設定頁進得去。文件改為明說這件事，並補上「兩處值不一致是正常狀態，盤點以設定頁的
末四碼為準」取代原本的「清掉以免誤導」。這也連帶修正了規劃階段「發現四（機密雙真相）」
的處理方向——那條發現本身成立，但當時想到的解法會製造更嚴重的問題。

其餘兩項：E5「加一台 EdgeProxy 收 webhook 那台完全不用改設定」補上 E1f 白名單的例外
（已啟用來源限制時要把 proxy 的 IP 加進清單，否則來源變成 proxy 後全被擋）。

### 程式碼終檢：4 項修正、1 項不採納

| 級別 | 發現 | 處置 |
|---|---|---|
| 高 | **暫時性 IO 失敗被當成檔案毀損**：`Read` 的 catch-all 不分 `IOException`，而監看重載與其他行程的 `File.Move` 沒有協調，撞上就是共用違規。後果不是「這次沒讀到」而是整份設定判成毀損、生效值退回 appsettings 的舊金鑰，且觸發它的檔案事件已用掉、不會有第二次來修正——**B2 想根治的「換金鑰間歇失效」會在 watcher 上原封重現** | `ReadAllBytesWithRetry`：只對 `IOException`／`UnauthorizedAccessException` 重試 3 次遞增退避；解密／反序列化失敗不走這條（那才是真毀損） |
| 中 | `_disposed` 非 volatile 且在鎖外設定：事件執行緒可在「Timer 已拆、旗標未設」的縫隙建一顆沒人釋放的新 Timer | 標 volatile，並把設定移進鎖內、拆 Timer 之前 |
| 中 | `_protector` 可被另一執行緒（`EdgeSettingsStore` 建構）換掉，而監看回呼會讀它，無記憶體屏障 | 標 volatile。**不採納**審查建議的「`SetProtector` 後觸發重載」——那會在 DI 建構期間打change token，為了一個正式環境不存在的情境（兩次都是 DPAPI）引入重入風險 |
| 低 | 測試重複斷言；測試建立的 provider 未釋放，其 watcher 活到暫存目錄被刪之後 | 刪掉重複斷言；provider 改 `using` |
| 低 | 兩個新檔缺檔尾換行 | 補上 |
| 中 | 建構式做 I/O 並啟動 watcher、釋放鏈隱含 | **不採納**：釋放靠 `ConfigurationManager` 的既有機制，1045 個測試建立大量 Edge host 無資源耗盡跡象；改動風險大於效益 |

確認無問題的：`MaxAsync` 空表回 null（投影型別可空，非可空才會拋）、門檻邊界與規格一致、
三態不改變任何原本會生效的值、`Read` 回傳型別變更的四個消費點全數更新、
`/edge-admin` 新增的建構參數只在 Edge 模式解析、watcher 測試確實繞過 `Save()`。

### 作業A 的技術事實修正（規格撰寫階段查證後定案）

規劃書原本把 API 形狀留給執行端決定（「既有端點改包裝或新增端點」）。寫規格前查證發現
前端 `settings.js` 把 `host-heartbeats` 當**裸陣列**消費（`rows.length`），改成包裝物件
會直接打斷畫面；而同一個面板已有 `database-status` 獨立端點、與 `host-heartbeats` 一起
用 `Promise.all` 併載的先例。因此規格直接把自由度收掉，指定新增獨立端點
`GET api/settings/message-flow`。

### 作業B 的落差：兩個冗餘別名（過度設計，已清掉）

agy 在 provider 上加了 `LastLoadStatus => LoadStatus`、在 `EdgeSettingsStore` 上加了
`Status => LoadStatus`，都是同一個值的第二個名字。前者只有測試在用（正式碼用的是
`LoadStatus`），後者完全沒有呼叫點。兩個都移除、測試改用 `LoadStatus`，重跑仍 1045 綠。

### 作業B 的規格外行為（判定：保留）

原本「檔案長度為 0」是靜默回空字典（視為正常），agy 把它歸進 `Unreadable`（記 Error、
顯示橫幅）。規格對三態的定義只寫了「解密或反序列化失敗」，嚴格說 0 位元組檔兩者皆非。
判定保留：原子寫入（先寫暫存檔再 `File.Move`）不可能產生 0 位元組檔，出現就是異常狀態，
讓它跟其他讀不出來的情況走同一條路徑（顯示、可復原）比靜默更正確。
