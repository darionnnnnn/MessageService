...
## 換模型體檢（Fable，2026-09-01，併 dev 後補做）

實作 agy、規劃／終檢／手改 Opus，本節體檢 Fable（收尾時漏了換模型這一步，使用者指出後補做）。
對 `83009ea..e218c55` 全 diff 重掃：規劃比對（定案 1~6 有對應實作、「明確不做」的
`LastWebhookAt` 只出現在 PLAN 文字、心跳鏈與資料層零改動——零 schema 契約成立）、
新測試逐條讀（斷言的是合併後生效值與真實 HTTP，非加密檔字典）、前端呼叫點與既有
錯誤處理同形、BOM 乾淨、1045 綠零警告。

**體檢重點放在終檢後的手改 commit `f3bce43`**（唯一沒被獨立視角看過的 delta），
抓到一項並修正：`ReadAllBytesWithRetry` 的 XML 註解插在 `Read` 既有註解之後，
把 `Read` 的整段文件註解變成貼在新方法頭上（雙重 `<summary>`），`Read` 自己反而沒了
註解。編譯零警告所以沒有任何訊號，靠逐行讀才看得到。已把兩段註解各歸其位。

低風險觀察（不修）：`ReadAllBytesWithRetry` 對 `FileNotFoundException`（`IOException`
子類）也會重試後歸入 `Unreadable`——檔案在 `File.Exists` 與讀取之間被刪才會發生，
沒有任何程式路徑會刪這個檔，人工刪除時多一行 Error 屬可接受行為。
