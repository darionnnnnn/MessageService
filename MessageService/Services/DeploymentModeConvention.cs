...
/// <summary>依目前部署模式（與 ingest API 金鑰是否配置）把不啟用的 controller 從
...
public class DeploymentModeConvention(DeploymentMode mode, bool ingestApiEnabled) : IApplicationModelConvention
{
    public void Apply(ApplicationModel application)
    {
        for (var i = application.Controllers.Count - 1; i >= 0; i--)
        {
            ...
            var modesAttribute = controllerType
                .GetCustomAttributes(typeof(EnabledInModesAttribute), inherit: true)
                .OfType<EnabledInModesAttribute>()
                .FirstOrDefault();
            if (modesAttribute is not null && !modesAttribute.Modes.Contains(mode))
            {
                application.Controllers.RemoveAt(i);
                continue;
            }

            var requiresApiKey = controllerType
                .GetCustomAttributes(typeof(RequiresIngestApiKeyAttribute), inherit: true)
                .Any();
            if (requiresApiKey && !ingestApiEnabled)
            {
                application.Controllers.RemoveAt(i);
            }
        }
