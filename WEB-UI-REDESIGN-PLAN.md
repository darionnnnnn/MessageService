...
> 狀態：**已實作完成**（2026-07-30，分支 `feature/line-style-web-ui`，尚未 merge 進 master）。
> 後端 commit `6731cb1`、前端 commit `7096e4a`。136 測試綠（33+103），並以 seeded 資料在瀏覽器
> 端到端驗證過群組切換、三種遮蔽模式＋完全匿名（含代號跨 reload 持久化）、側欄搜尋、
> 手機版列表↔聊天切換、字級記憶、連結化，過程 console 無錯誤、無失敗請求。
> 下方內容保留原始規劃供對照；一項偏差：Phase 4 的「SVG sprite」改用 emoji 字元圖示
> （沿用 chat.js 既有的 📄 檔案圖示慣例），更簡單且同樣零外部依賴。
...
| （體檢補記）兩個併發請求各為「不同的」新成員指派、又剛好雜湊到同一圖示時，理論上可能產生重複 Label（兩個「小熊」） | 已知理論限制、接受不修：本工具單一使用者＋IP 白名單，且兩請求需在毫秒級窗口內首次載入同群組的不同新成員才會發生；同群組同批成員的併發（如開兩個分頁）因處理順序與雜湊相同，算出的 Label 一致，不受影響 |
