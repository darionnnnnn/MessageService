...
public interface IContentWorkSource
{
    ...
    /// 的 Downloading 內容；若 <paramref name="startupAge"/> 有值（呼叫端行程已啟動多久），則額外包含該下載端掛名（ClaimedBy 為 <paramref name="ownerId"/>）
    /// 且 ClaimedAt 早於本行程啟動時刻（UtcNow - startupAge）但未逾期的 Downloading 內容（崩潰重啟回收孤兒，同時避免搶走重疊回收中兄弟行程正在進行的下載）；
    /// 並將回收內容改回 Pending 重新撿回。租約未逾期且非本下載端（或本行程啟動後建立）的 Downloading 則不予變動。</summary>
    Task<IReadOnlyList<long>> GetPendingIdsAsync(bool reclaimDownloading, TimeSpan? startupAge, string ownerId, CancellationToken cancellationToken);
    ...
    Task CompleteAsync(long contentId, Stream content, long contentLength, string? contentType, string ownerId, CancellationToken cancellationToken);

    Task FailAsync(long contentId, string ownerId, CancellationToken cancellationToken);
}
