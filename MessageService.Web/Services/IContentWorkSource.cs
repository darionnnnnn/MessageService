...
public interface IContentWorkSource
{
    ...
    /// 的 Downloading 內容；若 <paramref name="isStartup"/> 為 true，則額外包含該下載端掛名（ClaimedBy 為 <paramref name="ownerId"/>）
    /// 但未逾期的 Downloading 內容（模擬崩潰重啟回收）；並將回收內容改回 Pending 重新撿回。
    /// 租約未逾期且非本下載端（或週期重掃時的本下載端未逾期）Downloading 則不予變動。</summary>
    Task<IReadOnlyList<long>> GetPendingIdsAsync(bool reclaimDownloading, bool isStartup, string ownerId, CancellationToken cancellationToken);
