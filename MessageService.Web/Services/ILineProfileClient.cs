...
public interface ILineProfileClient
{
    ...
    Task<GroupSummary?> GetGroupSummaryAsync(string groupId, CancellationToken cancellationToken);

    /// <summary>Null if the member profile is unavailable (e.g. member left, privacy settings).</summary>
    Task<MemberProfile?> GetGroupMemberProfileAsync(string groupId, string userId, CancellationToken cancellationToken);
}
