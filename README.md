...
| `ContentDownload:ClaimLeaseMinutes` | 下載認領的租約分鐘數（預設 15）。認領時寫入 `MessageContents.ClaimedAt`／`ClaimedBy`（`ClaimedBy` 是**下載端站台**的身分：機器名＋站台目錄雜湊前 8 碼，同一站台跨行程重啟後不變，同機不同站台則不同；Edge 透過 ingest API 的 `ownerId` 參數帶給 Core），只有租約為空或已逾期的 Downloading 才會被回收改回 Pending——別台主機正在下載中的不會被誤收，卡住的下載也不必等重啟。**啟動掃描**額外回收掛在自己這個站台身分名下、且認領時間早於本行程啟動時刻的認領（那一定是上次行程留下的孤兒；同站台兄弟行程正在進行的下載必定晚於，不會被誤收），所以崩潰重啟不必等滿租約。完成／失敗時也會確認認領未被接手才寫結果，被接手後本站台的寫入結果會被放棄並記 Warning |
...
| `ProfileCache:BackfillIntervalMinutes` | 背景補刷掃描的間隔（預設 15 分鐘，0 表示只在啟動時掃一次）。挑出已過期、或有來源網址但圖檔尚未成功下載的群組／成員丟回刷新佇列，補上「安靜的群組不會被新訊息觸發刷新」的缺口；只挑側欄看得見的群組，群組最多佔一半名額、其餘給成員。只在 `Line:OutboundHere=true` 的主機上跑，拓撲差異見 [docs/DEPLOYMENT-MODES.md](docs/DEPLOYMENT-MODES.md) |
| `ProfileCache:BackfillMaxPerScan` | 單輪掃描最多入列的候選筆數（群組與成員合計，預設 50；0 或負數＝停用補刷） |
...
- **每台裝置各自的偏好（localStorage，刻意不進 DB）**：`chat-highlight-flow`（高亮邊框要不要流動，預設開）與 `chat-highlight-colors`（邊框顏色，`#rrggbb` 陣列，1–8 色，順序即漸層順序，預設四色為 LINE 綠／琥珀黃／珊瑚紅／紫）、`chat-highlight-opacity`（高亮強度，0.1–1.0、步進 0.05、預設 0.5；邊框漸層每個顏色都套這個透明度，光暈固定是它的 0.7 倍）。與字級、全版面同類：影響的是呈現而非資料是否外流
...
- **名稱與頭貼的就地更新**：後端尚未快取到的成員，訊息列先顯示 UserId ＋代號圖示，每則訊息帶 `profileResolved` 旗標告訴前端「目前模式下名稱與頭貼是否都已是最終值」（匿名模式看代號是否已指派、別名模式看有無別名、`Original` 模式還要等圖檔下載完成）。畫面上還有未解析的成員時，前端每 30 秒對 `api/groups/{groupId}/members/resolved` 查一次，回來後就地改名稱文字、補建頭貼節點——不重抓訊息、不重繪視窗、捲動位置不動；全部解析完即停止，同一成員最多試 10 輪（5 分鐘）就放棄。聊天標頭的群組名稱／頭貼／成員數跟著十秒的側欄刷新更新
- **頭貼**：成員頭貼只在 `Original` 模式顯示，走自家 API（`api/groups/.../members/.../avatar`），前端不直連 LINE CDN（載入失敗仍 fallback 代號圖示）；其他模式一律顯示伺服器指派的動植物代號圖示（emoji 渲染，前端 `ICON_EMOJI` 對照表需與後端 `AvatarIconCatalog` 的 IconKey 同步維護）。**群組照片不受名稱顯示模式限制，四種模式都顯示**——它是群組的識別，不是個人資料
- **字級**：設定「字體大小」數值輸入（px）＝聊天頁「中」檔泡泡文字的實際大小，小／大依比例（.87×／1.13×）跟著調整；聊天頁全部文字（不含頭貼/圖示）與設定 modal 本身的文字都吃同一份 `--font-base-px`（localStorage key `chat-font-base-px`，設在 `document.documentElement` 上、透過 inline style 覆寫、不寫死在樣式表裡，才不會被 CSS cascade 蓋掉）。設定 modal 跟聊天頁是同一個頁面，調字級時背後的聊天畫面會即時跟著變。範圍 8–28px、預設 20px；上下限與 localStorage 鍵只在 `chat.js` 的 `window.messageServiceFont` 定義一份，輸入框的 `min`/`max` 由 JS 依它填上。訊息頭貼與訊息列頂端留白跟著字級等比縮放，側欄群組頭貼、標頭頭貼、貼圖與圖片佔位固定不縮。設定 modal 本身另有 12px 下限，聊天頁不受此限
- 預設載入 3 天內對話，「載入更早 7 天」膠囊以最舊訊息 Id 當游標往前翻頁，沒有更早歷史時自動 disable。兩個「按了會沒反應」的空窗情況都有處理：畫面上一則訊息都沒有（沒有游標可用）時改成放大天數視窗重繪；群組沉寂比一個視窗還久時由 API 把視窗錨定到下一則更早訊息，保證每次點擊都會前進。膠囊（可按的「載入更早」與告知用的「沒有更早的訊息」皆同）只在捲到接近最頂部（40px 內）時才顯示
- 「回到最新」浮動按鈕：使用者往上捲動時自動退出跟隨模式並顯示未讀數，點擊或捲回底部即恢復跟隨並自動捲到新訊息
- **跟隨模式下的置底**：跟隨中的訊息列因媒體載入或待下載內容被替換而長高時，會自動重新釘回底部（`ResizeObserver` 與媒體 `load`/`error`/`loadedmetadata` 事件兩條路徑）；「載入更早」往前接進來的舊訊息不在觀察範圍，不會把畫面拉走。程式觸發的平滑捲動期間，跟隨狀態不由中途的捲動位置判定，抵達底部或逾時 1 秒才交還給捲動事件維護
- **訊息搜尋**：標頭 🔍 展開搜尋列（本群組／全部群組切換），比對訊息內容與發言者名稱，結果以 `<mark>` 高亮；點結果用 `aroundId` 跳轉到該訊息的上下文並閃爍定位，同時把視窗內符合的文字也標出來（標記會跳過連結內部，不會把 `<a>` 切開）。跳轉後進入「歷史檢視」，此時 3 秒訊息輪詢**只更新 Pending 內容狀態、不把新訊息接到視窗尾端**（避免時間軸斷層），畫面改顯示「回到最新」常駐按鈕，點擊會呼叫既有的群組選取流程整個重置回即時畫面
...
- **訊息高亮**：命中的訊息泡泡套漸層發光邊框。規則有兩種——關鍵字（範圍同遮蔽關鍵字）與指定人員（全部群組或單一群組，入口在訊息頭貼右鍵／長按）。**判定在前端做**（前端拿到的訊息文字已解密，不受加密搜尋視窗限制）。比對只針對訊息文字、不分大小寫。**遮蔽先於高亮**——被遮成 `*` 的詞前端已經比對不到，兩邊關鍵字重疊時高亮不會出現，設定頁有標註此事。命中的關鍵字文字本身另外包成 `<span class="highlight-keyword">`：粗體、比同一顆泡泡的文字大 1px、不分大小寫、標出全部出現處、不進入連結內部；多個關鍵字重疊時長的優先且不巢狀（例如同時有「緊急」與「緊急狀況」，只會標出「緊急狀況」）。一則訊息都沒命中時，DOM 與沒有這個功能時完全相同。呈現偏好見上方 localStorage 段；系統設定為減少動態效果時邊框靜止但仍發光
...
- 文字訊息中的網址會轉成可點連結（`target="_blank"` + `rel="noopener noreferrer"`）；所有訊息內容（含搜尋高亮、關鍵字標記）一律用 DOM 節點組裝（`textContent`／`createElement`），`innerHTML` 只用於清空容器，避免訊息內容造成 XSS
- **頁面內全螢幕**：在對話視窗的空白處按右鍵（手機長按）可切換，訊息泡泡、頭貼、群組項目與各種按鈕輸入框上不觸發，泡泡維持瀏覽器原生選單。全螢幕時側欄與底部唯讀輸入列隱藏，標頭縮矮並換成一條橫向可捲動的群組頭貼列：未讀數疊在頭貼右上角、與側欄共用同一份判定與 99+ 截斷，目前群組有選取樣式，點擊即可切換；重繪時保留橫向捲動位置。按 Esc 也可離開，但會先讓位給快捷選單、設定視窗與搜尋面板。**不使用瀏覽器的 Fullscreen API**，純粹是頁面內的版面模式；狀態不跨重新整理記憶
- **手機版（<768px）**：群組列表與聊天面板全螢幕切換，標頭出現「‹」返回鈕，仿 LINE 手機版導覽

**設定**：聊天頁裡的寬版 modal（`modal-xl`，手機自動轉全螢幕）。上方五個頁籤：介面顯示（字體大小 px 數值設定，見上方「字級」；「對話內容使用全版面寬度」勾選框，預設不勾＝桌面 2/3）、隱私與匿名（名稱顯示四模式，含完全匿名動植物代號；別名編輯器可依群組篩選成員；台灣個資自動遮蔽四開關——身分證/手機/市話預設開啟、健保卡預設關閉（12 碼數字跟宅配貨運單號撞格式）；訊息保留天數，異動有確認對話框防手滑）、關鍵字遮蔽規則（新增/刪除，預設等長 `*` 或自訂替換字串，全部群組或指定群組）、訊息高亮（關鍵字規則新增/刪除、人員規則列表與移除、流動開關、顏色色票與強度滑桿，見上方「訊息高亮」）、主機狀態（各部署主機的存活燈號／最後回報時間／outbox 積壓／加密金鑰指紋，讀 `HostHeartbeats` 表，指紋不一致時顯示警告，見下方資料表說明；另顯示全站「最後收到訊息」的時刻，見 `Monitoring:MessageSilenceWarnHours`）。多數變更即存（字體大小與對話寬度為 localStorage、不進 DB；PII 開關切換即 PUT；其餘 PUT 後顯示 toast），保留天數則需按「儲存」並過確認對話框才會寫入。資料只在**第一次**打開 modal 時才載入（`shown.bs.modal` 才打 API，不會讓聊天頁一開就多打一輪設定用的請求），成功寫入任何變更後關閉 modal 會自動重新整理目前的訊息視窗與側欄，不用手動重新整理頁面。

**API**（都在 `MessageService.Web/Controllers/Api/`）：

| 端點 | 用途 |
|---|---|
| `POST /api/groups/list` | 群組清單（僅列出有訊息的群組，名稱取自快取，無快取則顯示 GroupId；含最後訊息預覽〔已套遮蔽〕、最後訊息時間、成員數〔取自 `Groups.MemberCount`，未取得時回退為已快取的發言成員數〕、最後訊息 Id，依最後活動倒序）。body `{"read":{"群組Id":最後已讀Id}}`（可省略）帶各群組的已讀基準，回應附每群組未讀數（SQL 端計數並截斷在 100，前端顯示 99+；沒帶基準的群組未讀數為 0）。**基準走 POST body 而不是查詢字串**：長度隨群組數線性成長且沒有上界，`maxQueryString` 只是把門檻推遠、擋不住（換 nginx 等前置代理更是另一套限制）。缺 body 或 `read` 省略＝沒帶基準回 200；`read` 的值型別不對由模型繫結回 400 |
...
| `GET /api/groups/{groupId}/members/resolved?ids=` | 就地補齊訊息列的成員名稱與頭貼。`ids` 逗號分隔、上限 200 個（超過或空值回 400），回 `[{userId, displayName, pictureUrl, avatarIcon, profileResolved}]`；解析規則與訊息 DTO 共用同一段邏輯，匿名模式回代號不洩真名，且只對確實在群組裡的成員指派代號（`ids` 是外部輸入，不能拿它建立永久代號列） |
| `DELETE /api/groups/{groupId}/messages` | 刪除該群組的全部歷史訊息（附檔靠 CASCADE 一併刪除）。群組列、名稱／頭貼快取、成員、匿名代號、遮蔽與高亮規則都保留；訊息指標重算後（全刪光即為 null）群組不再出現在清單，直到有新訊息。回 200 帶刪除的訊息筆數（DTO 其餘欄位固定 0），群組不存在回 404 |
| `DELETE /api/groups/{groupId}` | 刪除整個群組：訊息、成員快取、匿名代號，以及三張裸 GroupId 規則表中屬於該群組的列（見下方資料表說明），最後刪 `Groups`（頭貼靠 CASCADE）。**`UserAliases` 與關鍵字本體不動**。bot 仍在該 LINE 群組時，下一則訊息會重建群組列，等同重置而非永久移除 |
...
以上四種模式（`Original` 除外）回應中一律不含 `PictureUrl`，即使前端不渲染也不外流，且**成員頭貼**的 API 會對非 `Original` 模式直接回 404 防止繞過遮蔽，因為頭貼與 URL 本身都是身分線索（群組照片的 API 不設此閘門：保護範圍是「人」，不是群組識別）。真實的 LINE URL 不會外流到前端，只存在資料庫的 `PictureFetchedUrl` 欄位供判斷頭貼是否更新（該欄位同時記錄「這個網址試過但永久拿不到」——圖檔超過上限或 404 時寫入網址但無對應圖檔，讓刷新流程不再重試同一個網址）。
...
| ClaimedBy | nvarchar(128), null | 認領的下載端站台身分（機器名＋站台目錄雜湊前 8 碼；舊版 Edge 沒帶時為 `legacy-edge`）；完成／失敗寫回時校驗認領未被接手；啟動時回收自己名下、認領時間早於本行程啟動時刻的孤兒（詳見設定表 `ContentDownload:ClaimLeaseMinutes`） |
...
**Groups** / **GroupMembers**：收錄端背景快取的群組名稱、成員顯示名稱與頭貼（7 天 TTL，來源是 LINE 的 group summary / member profile API），檢視端用來把 GroupId/UserId 轉成人看得懂的名稱。快取失敗時 fallback 顯示原始 ID；`ProfileCache:FailureRetryAfter`（預設 10 分鐘）冷卻期內失敗不會重複呼叫 LINE API。刷新寫入時名稱為空不覆寫既有值、`PictureUrl` 為空則刪掉圖檔並清中繼欄位；「有來源網址但圖檔尚未成功下載」視為過期（不看舊圖還在不在），由背景補刷（`ProfileCache:BackfillIntervalMinutes`）週期性撿回。`Groups.MemberCount`（`int`, null）存 LINE group members count API 回傳的群組真實人數，與 group summary 同一次刷新取得；為 null（尚未抓取或抓取失敗）時側欄改用已快取的發言成員數。加密開啟時群組名稱/顯示名稱/頭貼 URL 同樣走 `ENC2:` 整值加密。檢視端也會寫這張表：`Groups.LastMessageId` 指向的訊息若被保留期清除刪掉，`GroupsController.RecoverDriftedLastMessageAsync` 會即時查回目前真正的最後一則並修正這一列（見 `docs/DEPLOYMENT-GUIDE.md` 的 Viewer 帳號權限說明）。
...
| PictureFetchedUrl | nvarchar(max), null | 下載當時的來源 URL（用於判斷 LINE 頭貼是否更新）。圖檔永久拿不到（超過 2MB 上限、404）時也會寫入網址但沒有對應圖檔，代表「試過了，別再重試」 |
