...
- **每台裝置各自的偏好（localStorage，刻意不進 DB）**：`chat-highlight-flow`（高亮邊框要不要流動，預設開）與 `chat-highlight-colors`（邊框顏色，`#rrggbb` 陣列，1–8 色，順序即漸層順序，預設四色為 LINE 綠／琥珀黃／珊瑚紅／紫——聊天背景是中藍色，藍系會沉進背景所以預設不含藍）、`chat-highlight-opacity`（高亮強度，0.1–1.0、步進 0.05、預設 0.5；邊框漸層每個顏色都套這個透明度，光暈固定是它的 0.7 倍）。與字級、全版面同類：影響的是呈現而非資料是否外流
...
- **字級**：設定「字體大小」數值輸入（px）＝聊天頁「中」檔泡泡文字的實際大小，小／大依比例（.87×／1.13×）跟著調整；聊天頁全部文字（不含頭貼/圖示）與設定 modal 本身的文字都吃同一份 `--font-base-px`（localStorage key `chat-font-base-px`，設在 `document.documentElement` 上、透過 inline style 覆寫、不寫死在樣式表裡，才不會被 CSS cascade 蓋掉）。設定 modal 跟聊天頁是同一個頁面，調字級時背後的聊天畫面會即時跟著變。範圍 8–28px、預設 20px；上下限與 localStorage 鍵只在 `chat.js` 的 `window.messageServiceFont` 定義一份，輸入框的 `min`/`max` 由 JS 依它填上。訊息頭貼與訊息列頂端留白跟著字級等比縮放，側欄群組頭貼、標頭頭貼、貼圖與圖片佔位刻意固定（縮字是為了一次看更多訊息，不是把導航縮到點不到）。設定 modal 本身另有 12px 下限（控制面板縮到 8px 會不可讀），聊天頁不受此限
- 預設載入 3 天內對話，「載入更早 7 天」膠囊以最舊訊息 Id 當游標往前翻頁，沒有更早歷史時自動 disable。兩個「按了會沒反應」的空窗情況都有處理：畫面上一則訊息都沒有（沒有游標可用）時改成放大天數視窗重繪；群組沉寂比一個視窗還久時由 API 把視窗錨定到下一則更早訊息，保證每次點擊都會前進。膠囊只在捲到接近最頂部（40px 內）時才顯示——不論是可按的「載入更早」還是「沒有更早的訊息」，兩者在畫面中間出現都只是干擾；門檻取 40px 而非貼齊 0，是為了避免使用者停在臨界值附近時閃爍
- 「回到最新」浮動按鈕：使用者往上捲動時自動退出跟隨模式並顯示未讀數，點擊或捲回底部即恢復跟隨並自動捲到新訊息
- **跟隨模式下的置底**：新訊息接進畫面後內容還會繼續長高（圖片解碼完成、待下載內容被輪詢換成真實媒體），捲底如果只做一次就會停在半空中。跟隨中以 `ResizeObserver` 加媒體的 `load`/`error`/`loadedmetadata` 事件重新釘底——前者要等一個繪製框架，分頁被瀏覽器節流時不會觸發，兩條路徑並用才涵蓋得完整。**觀察只掛在往下接進來的列**：`prepend` 進來的是「載入更早」的舊訊息，使用者正停在上面讀，那些列撐高時把畫面拉到底會直接毀掉閱讀位置。程式觸發的平滑捲動期間另設旗標，抵達底部或逾時 1 秒才把跟隨狀態交還給捲動事件維護，避免動畫中途的位置被誤判成「使用者離開底部」
...
- **訊息高亮**：命中的訊息泡泡套漸層發光邊框。規則有兩種——關鍵字（範圍同遮蔽關鍵字）與指定人員（全部群組或單一群組，入口在訊息頭貼右鍵／長按）。**判定在前端做**：後端在加密開啟時只能比對最近數天、數百則，做在後端會讓高亮時有時無；前端拿到的訊息文字已是解密後的明文，不受影響。比對只針對訊息文字、不分大小寫。**遮蔽先於高亮**——被遮成 `*` 的詞前端已經比對不到，兩邊關鍵字重疊時高亮不會出現，設定頁有標註此事。命中的關鍵字文字本身另外包成 `<span class="highlight-keyword">`：粗體、比同一顆泡泡的文字大 1px、不分大小寫、標出全部出現處、不進入連結內部；多個關鍵字重疊時長的優先且不巢狀（例如同時有「緊急」與「緊急狀況」，只會標出「緊急狀況」）。一則訊息都沒命中時，DOM 與沒有這個功能時完全相同。邊框顏色、流動效果與強度都是每台裝置各自的偏好（見上），系統設定為減少動態效果時邊框靜止但仍發光
...
- **頁面內全螢幕**：在對話視窗的空白處按右鍵（手機長按）可切換，訊息泡泡、頭貼、群組項目與各種按鈕輸入框上不觸發（泡泡要留給瀏覽器原生的複製選單）。全螢幕時側欄與底部唯讀輸入列隱藏，標頭縮矮並換成一條橫向可捲動的群組頭貼列：未讀數疊在頭貼右上角、與側欄共用同一份判定與 99+ 截斷，目前群組有選取樣式，點擊即可切換；重繪時保留橫向捲動位置。按 Esc 也可離開，但會先讓位給快捷選單、設定視窗與搜尋面板。**不使用瀏覽器的 Fullscreen API**，純粹是頁面內的版面模式；狀態刻意不跨重新整理記憶——使用者未必知道要右鍵才能離開，重整是最直覺的逃生路徑
