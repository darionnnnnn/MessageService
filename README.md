...
以上四種模式（`Original` 除外）回應中一律不含 `PictureUrl`，即使前端不渲染也不外流，且 `AvatarsController` 會對非 `Original` 模式直接回 404 防止繞過遮蔽，因為頭貼與 URL 本身都是身分線索。真實的 LINE URL 不會外流到前端，只存在資料庫的 `PictureFetchedUrl` 欄位供判斷頭貼是否更新（該欄位同時記錄「這個網址試過但永久拿不到」——圖檔超過上限或 404 時寫入網址但無對應圖檔，讓刷新流程不再重試同一個網址）。
