...
- **部署與診斷**：部署模式後綴檔判別（恰一份/衝突/與鍵不一致擋啟動/未知後綴忽略）、記憶體環形緩衝與 `/proxy-admin/errors`、`/edge-admin` 三分頁（設定表單行為不變、錯誤排查三區塊、四網域連線測試含 EdgeProxy 拓撲的 host 標示）、outbound 失敗分類器（401/403/DNS/路由不可達/逾時/取消逐類）、空 token 守門與節流、Null 佇列首次丟棄警告、proxy 設定熱讀
