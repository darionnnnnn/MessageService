...
- **範圍與限制**：`groupId` 省略即搜尋全部群組；只搜文字訊息的 `Text` 欄（媒體訊息無文字可搜，檔名未經遮蔽管線、不搜）；文字命中與姓名命中**各有 50 筆獨立配額**（合併後上限 100 筆），依 `EventTimestamp` 新到舊排序，不做分頁。回應是 `MessageSearchResponseDto`（`results` ＋ `limitedByEncryption` 旗標），不是裸陣列。
...
`GET /healthz`（存活，不碰資料庫，恆 200）與 `GET /healthz/ready`（就緒，有資料庫的模式會 ping 一次，連不上回 503）。兩支在**所有部署模式**都存在，回應空 body，且**排除在 `Viewer:AllowedClientIps` 白名單之外**——監控系統與負載平衡器的來源 IP 通常不在辦公室 LAN 的白名單裡。監控設定建議見 [docs/DEPLOYMENT-GUIDE.md](docs/DEPLOYMENT-GUIDE.md)。
